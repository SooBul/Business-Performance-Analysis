...
select 
    ...
    O2.Segment
into Customer
from [Data - Superstore] O1 join [Data - Superstore] O2 on O1.Customer_ID = O2.Customer_ID
...
FROM 
    ...
    select 
        ...
        O2.Category,
        O2.Sub_Category
    from [Data - Superstore] O1 join [Data - Superstore] O2 on O1.Product_ID = O2.Product_ID
    ) as temp
...
-- Tạo table "Order_F" dựa trên table Order cũ, liên kết Order_F với các table vừa tạo
...
Select 
    ...
    O.Profit
Into [Order_N]
From [Data - Superstore] O JOIN Product P on O.Product_Name = P.Product_Name
...
FROM Address a JOIN [Order] o on a.Add_ID = o.Add_ID
group by a.state